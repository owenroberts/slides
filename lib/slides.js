...
var Slides = {
	...
	updateDrawingWidth: function() {
		...
		for (var i = 0; i < loadedCanvases.length; i++) {
			...
			// 0.96 for padding 2% // doesn't work going backward
			console.log(canvas.parentNode.offsetWidth, canvas.width);
			
			/* fuck me forever .... */
			var z = 1;
			if (canvas.parentNode.offsetWidth > canvas.width) {
				z =  (canvas.parentNode.offsetWidth * 0.96) / canvas.width;
			} else {
				z =   canvas.width / (canvas.parentNode.offsetWidth * 0.96);
			}
	...
	loadDrawing: function(slideNumber, data) {
		...
		Slides.slides[slideNumber].appendChild(canvas);
		console.log(Slides.slides[slideNumber].offsetWidth);
