...
var Slides = {
	setup: function() {
		...
		this.colors = { 
			...
			should be based on the generated background ...  */
	...
	updateDrawingWidth: function() {
		/* added drawings don't need to be upated .... */
		/* need to fix for height and width */
		var loadedCanvases = document.getElementsByClassName('loaded'); 
		for (var i = 0; i < loadedCanvases.length; i++) {
			// 0.96 for padding 2%
			var z =  (loadedCanvases[i].parentNode.offsetWidth * 0.96) / loadedCanvases[i].width;
			/* this all sucks */
			loadedCanvases[i].width = loadedDivs[i].parentNode.offsetWidth * 0.96;
			loadedCanvases[i].height *= z;
			loadedCanvases[i].getContext('2d').scale(z,z);
			loadedCanvases[i].getContext('2d').miterLimit = 1;
		}
	},
