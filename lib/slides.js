...
var Slides = {
	...
	updateDrawingWidth: function() {
		...
		/* fix all this crapppppp */

		for (var i = 0; i < drawingDivs.length; i++) {
			/* this sucks, need to detect drawings loaded before this shit */
			if (drawingDivs[i].tagName == "CANVAS") {
				var z =  (drawingDivs[i].parentNode.offsetWidth * 0.96) / drawingDivs[i].width;
				drawingDivs[i].width = loadedDivs[i].parentNode.offsetWidth * 0.96;
				drawingDivs[i].height *= z;
				drawingDivs[i].getContext('2d').scale(z,z);
				drawingDivs[i].getContext('2d').miterLimit = 1;
				drawingDivs[i].style.marginTop = 0; // terrible temp fix
			}
		}
		var loadedDivs = document.getElementsByClassName('loaded');
		for (var i = 0; i < loadedDivs.length; i++) {
			if (loadedDivs[i].tagName == "CANVAS") {
				// 0.96 for padding 2%
				var z =  (loadedDivs[i].parentNode.offsetWidth * 0.96) / loadedDivs[i].width;
				/* this all sucks */
				loadedDivs[i].width = loadedDivs[i].parentNode.offsetWidth * 0.96;
				loadedDivs[i].height *= z;
				loadedDivs[i].getContext('2d').scale(z,z);
				loadedDivs[i].getContext('2d').miterLimit = 1;
				loadedDivs[i].style.marginTop = 0; // terrible temp fix
			}
		}
	},
	...
	loadDrawing: function(slideNumber, data) {
		...
		canvas.getContext('2d').scale(z,z); // temp fix for zoom
	...
	this.drawLines = function() {
		...
		for (var h = 0; h < this.lines.length; h++) {
			...
			if (line.e) {
				...
				if (this.ctx.strokeStyle.replace("#","") != this.c) {
